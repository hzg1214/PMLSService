...
public class SeqNoAPIImpl implements ISeqNoAPI
{
    ...
    */
    private ResultData<String> getSeqNo(String typeCode, boolean update)
    {
        ...
        map.put("@upflag", update);
        seqNoDao.getSeqNoByTypeCode(map);
        
        if(null != map.get("@returnMsg") && map.get("@returnMsg") !=""){
        	String result = map.get("@returnMsg").toString();
        	int returnCode = null != map.get("@returnCode") ? (Integer)map.get("@returnCode") : 0;
        	if(returnCode==200){
            	seqNoResult.setReturnMsg("调用成功");
                seqNoResult.setReturnCode(ReturnCode.SUCCESS); 
            	seqNoResult.setReturnData(result);
        	}else{
        		seqNoResult.setReturnMsg(result);
        		seqNoResult.setReturnCode(ReturnCode.FAILURE);
        	}
        }else{
        	seqNoResult.setReturnMsg("规则未定义详细生成规则");
