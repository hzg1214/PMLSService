...
public class RemittanceTrackController extends BaseController {
	 ...
	 * 2020年4月7日
	 */
	@RequestMapping(value = "/querySwitchWeek/{param}", method = RequestMethod.GET)
	public String querySwitchWeek(@PathVariable String param) {

		Map<String, Object> queryParam = JsonUtil.parseToObject(param, Map.class);
		ResultData<PmlsPerformSwitchWeekDto> resultData = new ResultData();
		try {
			resultData = remittanceTrackService.querySwitchWeek(queryParam);
		} catch (Exception e) {
			resultData.setFail();
			logger.error("remittanceTrackService", "RemittanceTrackController", "querySwitchWeek", "",
					null, "", "导入时查询关账数据失败", e);
		}
		return resultData.toString();
	}
	
	@RequestMapping(value = "/querySwitchFinishWeek/{param}", method = RequestMethod.GET)
	public String querySwitchFinishWeek(@PathVariable String param) {
		
		Map<String, Object> queryParam = JsonUtil.parseToObject(param, Map.class);
		ResultData<PmlsWeekInfo> resultData = new ResultData();
		try {
			resultData = remittanceTrackService.querySwitchFinishWeek(queryParam);
		} catch (Exception e) {
			resultData.setFail();
			logger.error("remittanceTrackService", "RemittanceTrackController", "querySwitchFinishWeek", "",
					null, "", "导入时查询关账数据失败", e);
		}
		return resultData.toString();
	}
     ...
     * 2020年4月7日
     */
	@RequestMapping(value = "/getNums/{param}", method = RequestMethod.GET)
	public String getNums(@PathVariable String param) {
		
		ResultData resultData = new ResultData<>();
		// 获取请求参数
		Map<String, Object> queryParam = JsonUtil.parseToObject(param, Map.class);
		try {
			Integer size = remittanceTrackService.getNums(queryParam);
			resultData.setReturnData(size);
		} catch (Exception e) {
			resultData.setFail();
			logger.error("remittanceTrack", "RemittanceTrackController", "getNums", "", -1, "", "回款跟踪数据-周数", e);
		}
		return resultData.toString();
	}
}
