...
public class CooperationService extends BaseService {
    ...
    public ResultData addCooperation(CooperationDto dto) throws Exception {
        ...
        if (dto.getViolateAmount() != null) {
            ...
            BigDecimal bd = dto.getViolateAmount().multiply(baseAccount).setScale(2, BigDecimal.ROUND_HALF_UP);
            dto.setViolateAmount(bd);
        }
...
//        fileRecordMainMapper.updateStatusByContractId(Integer.parseInt(dto.getId().toString()));
    ...
    public ResultData getCooperationInfo(CooperationDto dto) {
        ...
        if (cooperationDto != null && cooperationDto.getId() != 0) {
            if (cooperationDto.getViolateAmount() != null) {
                BigDecimal baseAccount = new BigDecimal("10000");
                BigDecimal bd = cooperationDto.getViolateAmount().divide(baseAccount).setScale(2, BigDecimal.ROUND_HALF_UP);
                cooperationDto.setViolateAmount(bd);
            }
    ...
    public ResultData checkCompany(BusinessManagerDto dto) throws Exception {
        ...
        if (businessManagerDto != null &&
                businessManagerDto.getArteryType() != null &&
                businessManagerDto.getArteryType() == 29501
                ) {
        ...
        queryParam.put("vendor_name",businessManagerDto.getCompanyName());
        BusinessManagerDto vendorDto = cooperationMapper.getCompanyVendorInfo(queryParam);
        if (vendorDto == null) {
    ...
    public ResultData queryEstateHtRecord(Map<String, Object> queryParam) throws Exception {
        ...
        logger.info("分销合同#获取项目合同列表#Service start queryParam="+JSON.toJSONString(queryParam));
        List<EstateContractDto> list = cooperationMapper.queryEstateHtRecord_ByPrd(queryParam);       
        if (list != null) {
            int size = Integer.parseInt(queryParam.get("total").toString());
            resultData.setTotalCount(String.valueOf(size));
            resultData.setReturnData(list);
        }
        logger.info("分销合同#获取项目合同列表#Service end queryParam="+JSON.toJSONString(queryParam));
//        resultData.setTotalCount((String) queryParam.get(QueryConst.TOTAL_COUNT));
////        resultData.setReturnData(list);
//       
//        List<EstateContractDto> list = cooperationMapper.queryEstateHtRecord(queryParam);
...
//        resultData.setReturnData(list);
    ...
    public ResultData sendOACooperation(HttpServletRequest request, CooperationDto dto) throws Exception {
        ...
        if (StringUtil.isEmpty(oldDto.getSignType())
                ||StringUtil.isEmpty(oldDto.getHtType())
                ||StringUtil.isEmpty(oldDto.getHtCategory())
                ||oldDto.getDybl()==null
                ||StringUtil.isEmpty(oldDto.getContractNameType())
        ) {
