...
import cn.com.eju.deal.otherReport.dao.LnkYjQtYjsrMapper;
import cn.com.eju.deal.otherReport.dao.LnkYjQtYjssMapper;
import cn.com.eju.deal.otherReport.dao.LnkYjQtYssrMapper;
...
public class LnkYjQtWyService extends BaseService {
    ...
    @Resource
    LnkYjQtYjsrMapper lnkYjQtYjsrMapper;
    @Resource
    LnkYjQtYssrMapper lnkYjQtYssrMapper;
    @Resource
    LnkYjQtYjssMapper lnkYjQtYjssMapper;
    @Resource
    CommonMapper commonMapper;

    public ResultData<List<LnkYjWyDto>> getLnkYjQtWyList(Map<String, Object> map) throws Exception {
        ResultData<List<LnkYjWyDto>> resultData = new ResultData<>();
        List<LnkYjWyDto> relList = new ArrayList<>();
        ...
        if ("yjsr".equals(estateType)) {
            relList = lnkYjQtYjsrMapper.getList(map);
        } else if ("yssr".equals(estateType)) {
            relList = lnkYjQtYssrMapper.getList(map);
        } else if ("yjss".equals(estateType)) {
            relList = lnkYjQtYjssMapper.getList(map);
        }
    ...
    public ResultData<String> insertLnkQtWy(LnkYjWyDto dto) throws ParseException {
        ResultData resultData = new ResultData();
        List<LnkYjWyDto> lnkYjWyDtoList = dto.getLnkYjWyDtoList();
        int count = 0;
        boolean flag = false;
        String msg = "";
        String gzLastMonth = "";
        String estateType = lnkYjWyDtoList.get(0).getEstateType();
        Integer empId = new Integer(0);
        if (lnkYjWyDtoList != null && lnkYjWyDtoList.size() > 0) {

            List<Map<String, Object>> checkRelMapList = new ArrayList<>();


            empId = Integer.valueOf(lnkYjWyDtoList.get(0).getCrtEmpId());
            //获取当前城市关账到的月份
            Map<?, ?> map = commonMapper.checkCitySwitchMonth(lnkYjWyDtoList.get(0).getCityNo());
            ...
            for (LnkYjWyDto cDto : lnkYjWyDtoList) {
            ...
            for (LnkYjWyDto lDto : lnkYjWyDtoList) {
                    ...
                    if (switchFlag == null || switchFlag <= 0) {
                        ...
                        if (closeLastDay != null && closeLastDay.getTime() <= recordDate.getTime()) {
                            if (estateType.equals("yjsr")) {
                                lnkYjQtYjsrMapper.mergeInsertWy(lDto);
                            } else if (estateType.equals("yssr")) {
                                lnkYjQtYssrMapper.mergeInsertWy(lDto);
                            } else if (estateType.equals("yjss")) {
                                lnkYjQtYjssMapper.mergeInsertWy(lDto);
                            }
                        } else {
                            flag = true;
                        }
        ...
        if (flag) {
            ...
            resultData.setReturnMsg("部分数据导入成功！" + msg + "无需手动导入外佣信息！");
        } else {
